...
/**
 * Controls the application flow and uses
 SecurityService + Database
 * to perform registration, login, lockout,
...
public class SecureLoginSystem {
    ...
    private void register() {
        ...
        String password =
                scanner.nextLine();
        if
        (!securityService.isPasswordStrong(password
        )) {
    ...
    private void login() {
        ...
        SecurityService.LoginResult result
                = securityService.attemptLogin(username,
                password);
    ...
    private void adminMenu() {
        ...
        if (choice.equals("1")) {
            ...
            String target =
                    scanner.nextLine().trim();
            if
            (securityService.unlockUser(target)) {
            ...
            List<LoginAttempt> attempts =
                    database.getRecentAttempts(target, 10);
            if (attempts.isEmpty()) {
